...
import models
# Delegated
from template.runner.image_classification_full_image import evaluate, train
from template.runner.image_classification_full_image.setup import set_up_model, set_up_dataloaders
from util.misc import checkpoint, adjust_learning_rate


class ImageClassificationFullImage:
    @staticmethod
    def single_run(writer, current_log_folder, model_name, epochs, lr, decay_lr,
        ...
        ImageClassificationFullImage._validate_model_input_size(model_expected_input_size, model_name)
        ...
        val_value[-1] = ImageClassificationFullImage._validate(val_loader, model, criterion, writer, -1, **kwargs)
        for epoch in range(start_epoch, epochs):
            # Train
            train_value[epoch] = ImageClassificationFullImage._train(train_loader, model, criterion, optimizer, writer, epoch,
                                                            **kwargs)

            # Validate
            if epoch % validation_interval == 0:
                val_value[epoch] = ImageClassificationFullImage._validate(val_loader, model, criterion, writer, epoch, **kwargs)
        ...
        # Test
        test_value = ImageClassificationFullImage._test(test_loader, model, criterion, writer, epochs - 1, **kwargs)
